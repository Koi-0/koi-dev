...
const projects: {
    ...
    },
    {
        ...
        description:
            "곡물 트레이더가 예측 결과와 트랙레코드를 확인하는 B2B 대시보드입니다. 핵심 사용자 흐름, 조직 기반 RLS 멀티테넌시, mock·실데이터 공통 접근 구조를 설계했습니다.",
        ...
        <div className='flex flex-1 flex-col px-6 py-14 sm:px-10'>
            <main className='mx-auto w-full max-w-xl flex-1'>
                <header>
                    ...
                    <h1 className='mt-3 break-keep text-3xl font-semibold tracking-tight sm:text-4xl'>프로젝트</h1>
                </header>

                {projects.map((project) => {
                    const body = (
                        <>
                            <p className='font-mono text-sm tracking-wide text-accent'>{project.number}</p>
                            ...
                            <p className='mt-2 font-mono text-sm tracking-wide text-muted'>{project.period}</p>
                            <p className='mt-6 break-keep text-lg leading-relaxed text-muted'>{project.description}</p>
                            <p className='mt-4 font-mono text-sm tracking-wide text-muted'>{project.keywords}</p>
                        </>
                    );

                    return (
                        <section key={project.number} className='mt-16 border-t border-border pt-16'>
                            {project.href ? (
