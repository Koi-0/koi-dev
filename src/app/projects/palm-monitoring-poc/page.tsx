...
const overview: [string, string][] = [
    ...
    ["담당 역할", "Product Definition · Web Application · BFF · Backend/Infra Audit · Operations"],
    [
        "협업 구조",
...
const architectureScope: Record<ArchitectureScope, { label: string; className: string }> = {
    external: {
        label: "협업·외부",
...
const outcomes = [
    {
        result: "국가에서 농장까지 탐색하고 NDVI·NDMI·TCI 및 30일 시계열을 실제 API로 조회하는 흐름을 완성했습니다.",
        verification: "배포 환경에서 사용자 흐름을 반복 실행하고 네트워크 응답과 화면 표시를 함께 확인했습니다.",
    },
    {
        result: "외부 인증 정보를 브라우저 밖에 둔 BFF 경계를 구현했습니다.",
        verification: "배포된 브라우저 번들을 점검해 인증 비밀 값이 포함되지 않은 것을 확인했습니다.",
    },
    {
        result: "고객 시연 범위의 UI와 데이터 흐름을 안정화하고 경영진 시연을 완료했습니다.",
        verification:
            "브라우저 자동화로 핵심 흐름을 반복 실행한 뒤 결과 화면, 콘솔과 접근 제어를 직접 검토했습니다. 확인할 데이터가 없는 항목은 통과가 아닌 확인 불가로 구분했습니다.",
    },
    {
        result: "인계 시스템의 운영 위험을 정리하고 우선순위가 높은 항목을 국소 수정했습니다.",
        verification: "코드·설정·라이브 환경을 교차 확인하고 신규 환경에서 핵심 처리 흐름을 다시 실행했습니다.",
    },
];
...
const headingClass = "mt-3 break-keep text-2xl font-semibold tracking-tight sm:text-3xl";
    ...
    return (
        <figure className='mt-8 border border-border p-5 sm:p-6'>
            ...
            <div className='mt-5 flex flex-wrap gap-x-4 gap-y-2 font-mono text-xs text-muted'>
                {(Object.keys(architectureScope) as ArchitectureScope[]).map((scope) => (
                    <span key={scope} className='inline-flex items-center gap-2'>
                        <span aria-hidden className={`h-3 w-3 border ${architectureScope[scope].className}`} />
                            ...
                            <div className={`border p-4 ${scope.className}`}>
                                <div className='flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4'>
                                    <div>
                                        ...
                                        <p className='mt-1 break-keep text-sm text-muted'>{node.detail}</p>
                                    </div>
                                    <span className='shrink-0 font-mono text-xs text-muted'>{scope.label}</span>
                                </div>
                            </div>
                            {index < architectureNodes.length - 1 ? (
                                <div aria-hidden className='py-2 text-center font-mono text-sm text-muted'>
                                    ↓
                                </div>
                            ) : null}
                        </li>
        ...
        <div className='flex flex-1 flex-col px-6 py-14 sm:px-10'>
            <main className='mx-auto w-full max-w-xl flex-1'>
                <header>
                    ...
                    <p className='mt-2 font-mono text-sm tracking-wide text-muted'>2026.05 – 2026.07</p>
                    <p className={`mt-8 ${bodyClass}`}>
                        대기업 종합상사가 팜유 산지의 작황 변화를 위성 지표로 조기에 파악하고, 이를 조달 판단에 활용할
                        수 있는지 검증한 PoC입니다.
                    </p>
                </header>
                ...
                <section className={sectionClass}>
                    ...
                    {background.map((paragraph, index) => (
                        <p key={paragraph} className={`${index === 0 ? "mt-8" : "mt-4"} ${bodyClass}`}>
                            {paragraph}
                        </p>
                    ))}
                </section>
                        ...
                        <div key={item.title} className={index === 0 ? "mt-8" : "mt-12"}>
                            ...
                            {item.steps.map((step) => (
                                <div key={step.label} className='mt-6'>
                                    ...
                                    {step.body ? <p className={`mt-2 ${bodyClass}`}>{step.body}</p> : null}
                                    {step.items ? (
